    ...
    >
      <div className=" h-[63%] flex w-full md:w-4/6 justify-between">
        {FooterItem.map((item, index) => (
          <div key={index} className="w-full md:w-1/2 lg:w-1/3 mb-6 text-charcoal_gray font-bold mini:text-xs mid:text-sm dark:text-cloud_grey">
              ...
              {item.link.map((link, i) => (
                <NavLink key={i} className="block pt-2 hover:text-ocean_blue transition duration-300 whitespace-nowrap">
                  {link}
                </NavLink>
              ))}
          </div>
      ...
      <div className="md:w-1/3 mini:w-11/12 h-[70%] bg-white dark:bg-midnight_blue rounded-2xl py-6 px-10 ">
      ...
      <div className="flex flex-row gap-1 md:absolute md:top-72 mini:top-96 md:right-0 dark:text-white">
